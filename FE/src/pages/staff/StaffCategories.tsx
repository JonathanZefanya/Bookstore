...
const StaffCategories: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you certain? Books linked might cause errors unless managed.')) return;
    try {
  ...
  const openForm = (cat?: Category) => {
    setForm(cat ? { id: cat.id, name: cat.name, description: cat.description || '' } : { id: 0, name: '', description: '' });
    setModalOpen(true);
  };
            ...
            <tbody className="divide-y divide-gray-200 bg-white text-sm">
              {categories.map(c => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-gray-500 font-mono">{c.id}</td>
                  <td className="px-6 py-4 font-bold text-gray-900">{c.name}</td>
                  <td className="px-6 py-4 text-gray-500 font-mono text-xs">{c.slug}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => openForm(c)} className="text-indigo-600 hover:text-indigo-900 mr-4"><PencilIcon className="w-5 h-5"/></button>
                    <button onClick={() => handleDelete(c.id)} className="text-red-500 hover:text-red-700"><TrashIcon className="w-5 h-5"/></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b border-gray-100 font-bold text-lg bg-gray-50">{form.id ? 'Edit Category' : 'New Category'}</div>
            <form onSubmit={handleSub} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea rows={3} value={form.description} onChange={e=>setForm({...form, description: e.target.value})} className="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 px-3 py-2" />
              </div>
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                ...
                <button type="submit" className="px-4 py-2 bg-indigo-600 text-white font-bold rounded-lg shadow hover:bg-indigo-700">Save Category</button>
              </div>
            </form>
          </div>
        </div>
